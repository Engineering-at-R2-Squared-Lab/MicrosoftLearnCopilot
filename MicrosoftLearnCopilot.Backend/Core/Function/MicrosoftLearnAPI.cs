...
public class MicrosoftLearnAPI
{
    ...
    public async Task<List<MicrosoftLearnModel.LearningPathItem>> getLearningPath(string query)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            int fuzzyScoreThreshold = 30;

            var filteredLP = results?
                .Where(r => r.Score > fuzzyScoreThreshold)
                .Select(r => allItems?[r.Index])
                .Take(5)
                .ToList();

            Console.WriteLine($"Found {filteredLP?.Count} learning paths matching the query with score > 55 '{query}'.");
